...
>
> **Status (2026-08-26)**: All eight items completed. Each item passed a task-scoped spec+quality review; a final whole-branch review (base 06f363d) returned one Important finding — the O(4ⁿ) doc warning on the new exported Bloch API — fixed and re-review-verified in 57422eb alongside three doc/comment nits. Full suite, `go vet`, gofmt, and staticcheck clean at the branch head.
...
- **Result (2026-08-26)**: ✅ Done — wired in, not deprecated. `checkCapabilities` (`circuit/circuit.go:159-186`) now enforces `MaxGateQubits()` as a hard upper bound after the `SupportsGateQubits` primary check, honoring the documented 0-means-unbounded semantics (dense returns 0, sparse 2); violations return the same `*quantum.UnsupportedOperationError` class with identical `Operation` formatting, before any state mutation. Doc comment states both halves of the contract are enforced. Table-driven `TestExecuteEnforcesMaxGateQubits` covers violation (typed error + behaviorally-asserted no-mutation via an applyGateCalled flag), boundary (gate size == max, passes), and under-limit cases, plus a public `CheckBackendCapabilities` entry-point test. Commit c779ea6.
...
- **Result (2026-08-26)**: ✅ Done — both the minimum and the optional guard. `validateIndependentStates` doc comment states the comparable-dynamic-type requirement (and why pointer-receiver backends are safe); `ExecuteAllParallel`'s exported SAFETY paragraph carries the same contract. Cold-path `reflect.TypeOf(exec.State).Comparable()` guard runs before any map insertion and returns a new taxonomy-conformant `quantum.UncomparableStateError{Index, TypeName}` (`quantum/errortypes.go` — no existing type fit; `SharedStateError` presupposes comparability). Table-driven test proves the typed error instead of a panic for a slice-bearing value-receiver state, with a `recover()` guard making any panic a test failure, and pointer states still validating. Commits b24d06e, 5e34dea.
...
- **Result (2026-08-26)**: ✅ Done. `density.FromState(s quantum.QuantumState) (*Matrix, error)` builds ρ = |ψ⟩⟨ψ| via the interface's `Amplitude` accessor (returns the package's existing `*Matrix` type rather than the proposed `*State` — no renames), delegating qubit-count validation to `New` per ADR-0004; `visualization.BlochVectorFromState(s, target)` bridges it to `ReducedBlochVector` with a typed `QubitsOutOfRangeError` guard placed before the O(4ⁿ) allocation. All four required test groups landed table-driven, each multi-qubit case run against both dense and sparse backends: |0⟩⊗|+⟩ → (0,0,1)/(1,0,0), Bell pair → zero vector on both halves, single-qubit agreement with `BlochVectorFromQubit` including a magnitude-asymmetric phased case that pins the y-sign convention. Reviewer independently re-derived the outer-product conjugation side (matches the package's pre-existing helper), the partial-trace index arithmetic, and every test expectation. The O(4ⁿ) cost warning initially lived only on `FromState`'s internal doc; the final whole-branch review flagged that and the exported entry point now carries it too (57422eb). Commits 2680967, db393da, 57422eb.
...
- **Result (2026-08-26)**: ✅ Done — documented, not implemented. ADR-0007 (`docs/adr/0007-density-backend-scope.md`, Accepted; indexed in the ADR README) records that `internal/density` is deliberately a noise-and-analysis backend (Kraus channels, reduced Bloch vectors, `FromState` bridge) and does NOT implement `quantum.QuantumState`, with the revisit trigger spelled out: a concrete consumer needing to run full circuits on density matrices interchangeably. Package doc comment on `internal/density/state.go` states the scope and points at the ADR. Rationale for choosing the document path over the implement path: measurement/collapse semantics on density matrices is real design work with no current consumer, and the package now has two real consumers in its current shape (noise demo, 4.5's Bloch bridge). Commit ddbcad7.
...
- **Result (2026-08-26)**: ✅ Done. `internal/examples/utils.go` gained exported `PrintRule(width)` / `PrintBanner(width, titles...)`; the hand-inlined `=====` blocks in `bell.go`, `hadamard.go`, `tgate.go`, and both `cmd/quantum/main.go` blocks (opening two-title and closing banner of the `all` case) are now helper calls. Output byte-identity verified twice over: the implementer diffed before/after `quantum bell/hadamard/tgate/-no-pause all` runs (only pre-existing measurement-randomness lines differ; banner lines extracted and compared identical) and the reviewer independently re-derived it from the diff — programmatic `=`-count checks (45-wide example rules, 56-wide CLI rules), verbatim title-string equality, and a byte-trace of the `Println("\n"+rule)` → `Println(); PrintBanner(...)` split. ADR-0006 contract tests green. Commit 7a64b79.
