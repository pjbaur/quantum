...
- Identify the current behavior and limitations in `state.ApplyGate`.
- Define the correct indexing and bit-order conventions used by the project.
- Implement multi-qubit gate application for arbitrary target qubit indices.
- Ensure correct handling of non-adjacent qubits and multi-target gates.
- Verify unitarity preservation and amplitude conservation in the update logic.
- Add clear error handling for invalid indices or mismatched gate sizes.
- Keep single-qubit behavior unchanged to avoid regressions.
- Add or update documentation comments if behavior is subtle.

## 2) Bell state validation tests

- Add a table-driven test for Bell state preparation.
- Cover at least the standard sequence: H on qubit 0, then CNOT(0 -> 1).
- Validate amplitudes (including phase) for the four Bell states.
- Verify measurement probabilities match expected distributions.
- Ensure tests are deterministic and avoid debug prints.
- Place tests alongside existing state or gate tests, following repo conventions.
